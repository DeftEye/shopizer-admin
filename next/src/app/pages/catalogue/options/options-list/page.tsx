...
import { deleteOption, listOptions } from "@/lib/catalogue/options-api";
import {
  nameFromDescriptions,
  readApiErrorMessage,
} from "@/lib/catalogue/options-helpers";
import type { OptionsListParams, ProductOption } from "@/lib/catalogue/options-types";
...
export default function OptionsListPage() {
  ...
  const fetchList = useCallback(
    async (next: OptionsListParams) => {
      setLoading(true);
      try {
        const result = await listOptions(next);
  ...
  }, [lang, nameFilter, options, params.lang]);

  async function onRemove(item: ProductOption) {
    if (!item.id) {
      return;
    }
    ...
    try {
      ...
      setBanner({ kind: "success", text: t("OPTION.OPTION_REMOVED") });
      void fetchList(params);
    } catch (error) {
      setBanner({
        kind: "error",
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <h1 className={styles.title}>{t("COMPONENTS.OPTIONS_LIST")}</h1>
        <div className={styles.toolbar}>
          ...
          <Link
            ...
            href="/pages/catalogue/options/create-option"
          >
            {t("COMPONENTS.CREATE_OPTION")}
          </Link>
        </div>
      </header>
            ...
            <tbody>
              {visible.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>{item.type ? t(`COMMON.${item.type}`) : ""}</td>
                  <td>
                    <div className={styles.actions}>
                      <button
                        ...
                        onClick={() =>
                          router.push(
                            `/pages/catalogue/options/option/${item.id}`,
                          )
                        }
                      >
                        {t("COMMON.EDIT")}
